...
export default class {
	...
	protected read() {
		return fs.readFileSync(this.args.filePath);
	}
	...
	protected write(content: string): FileWriteResponse {
		...
		if (this.args.filePath === outputPath) {
			Logger.log(`Because output path is the same as original file path, it is going to backup the original file automatically.`)
